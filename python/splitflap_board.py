#!/usr/bin/env python3
"""
splitflap_board.py — Full-screen split-flap (airport-style) text board for a
Raspberry Pi Zero 2 W. Fetches a few lines of text from a URL and animates them
onto a mechanical split-flap display with a true two-halves rotating flip: the
top card folds down over the bottom with vertical foreshortening, like a real
Solari board. No browser required.

Tested target: Python 3.13.5 on Raspberry Pi OS (Bookworm), running under X.

Dependencies:
    pip install pygame
    # or, via apt on Raspberry Pi OS:
    #   sudo apt install python3-pygame

Run:
    python3 splitflap_board.py https://example.com/board.txt

    # options
    python3 splitflap_board.py URL --cols 32 --rows 6 --interval 60 --windowed
...
class GlyphCache:
    ...
    the same size so the Zero 2 W only rasterizes glyphs once."""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Full-screen split-flap text board.")
